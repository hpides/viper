...
namespace viper::alex {
    ...
    public:
        ...
        typedef std::pair<KeyType, P> V;

        // ALEX class aliases
        typedef Alex<KeyType, P, Compare, Alloc, allow_duplicates> self_type;
        typedef AlexModelNode<KeyType, P, Alloc> model_node_type;
        typedef AlexDataNode<KeyType, P, Compare, Alloc, allow_duplicates> data_node_type;
/*
        P Insert(const KeyType &, P, std::function<bool(KeyType, index::KeyValueOffset)>) override;

        P Insert(const KeyType &, P) override;

        P Get(const KeyType &, std::function<bool(KeyType, index::KeyValueOffset)>) override;

        P Get(const KeyType &) override;*/
        ...
        AlexNode<KeyType, P> *root_node_ = nullptr;
   ...
   */
        struct InternalStats {
            KeyType key_domain_min_ = std::numeric_limits<KeyType>::max();
            KeyType key_domain_max_ = std::numeric_limits<KeyType>::lowest();
   ...
   * (base_cost). */
    ...
    private:
        ...
        AlexNode<KeyType, P> *copy_tree_recursive(const AlexNode<KeyType, P> *node) {
            ...
            if (node->is_leaf_) {
                ...
                while (cur < node_copy->num_children_) {
                    AlexNode<KeyType, P> *child_node = node_copy->children_[cur];
                    AlexNode<KeyType, P> *child_node_copy = copy_tree_recursive(child_node);
...
// node's parent.
        ...
        forceinline data_node_type *get_leaf(
                KeyType key, std::vector<TraversalNode> *traversal_path = nullptr) const {
            if (traversal_path) {
                traversal_path->push_back({superroot_, 0});
            }
            AlexNode<KeyType, P> *cur = root_node_;
            if (cur->is_leaf_) {
    ...
    private:
        ...
        inline void correct_traversal_path(data_node_type *leaf,
            ...
            if (left) {
                ...
                if (start_bucketID == 0) {
                    ...
                    AlexNode<KeyType, P> *cur = parent->children_[correct_bucketID];
                    while (!cur->is_leaf_) {
                        auto node = static_cast<model_node_type *>(cur);
        ...
        KeyType get_min_key() const { return first_data_node()->first_key(); }

        // Returns maximum key in the index
        KeyType get_max_key() const { return last_data_node()->last_key(); }
    ...
    private:
        ...
        void delete_node(AlexNode<KeyType, P> *node) {
            if (node == nullptr) {
                return;
        ...
        // True if a == b
        template<class K>
        forceinline bool key_equal(const KeyType &a, const K &b) const {
    ...
    private:
        ...
        void create_superroot() {
            ...
            superroot_->children_ =
                    new(pointer_allocator().allocate(1)) AlexNode<KeyType, P> *[1];
        ...
        // dense keys.
        void bulk_load_node(const V values[], int num_keys, AlexNode<KeyType, P> *&node,
                            int total_keys,
                            const LinearModel<KeyType> *data_node_model = nullptr) {
            ...
            if (experimental_params_.fanout_selection_method == 0) {
                ...
                best_fanout_stats = fanout_tree::find_best_fanout_bottom_up<KeyType, P>(
                ...
                best_fanout_stats = fanout_tree::find_best_fanout_top_down<KeyType, P>(
                        values, num_keys, node, total_keys, used_fanout_tree_nodes,
            ...
            if (best_fanout_tree_cost < node->cost_ ||
                ...
                if (best_fanout_tree_depth == 0) {
                    ...
                    int max_data_node_keys = static_cast<int>(
                            derived_params_.max_data_node_slots * data_node_type::kInitDensity_);
                    fanout_tree::compute_level<KeyType, P>(
                            values, num_keys, node, total_keys, used_fanout_tree_nodes,
                ...
                model_node->children_ =
                        new(pointer_allocator().allocate(fanout)) AlexNode<KeyType, P> *[fanout];
                ...
                for (fanout_tree::FTNode &tree_node: used_fanout_tree_nodes) {
                    ...
                    LinearModel<KeyType> child_data_node_model(tree_node.a, tree_node.b);
        ...
        data_node_type *bulk_load_leaf_node_from_existing(
            ...
            if (tree_node) {
                ...
                // recompute it
                LinearModel<KeyType> precomputed_model(tree_node->a, tree_node->b);
                ...
                LinearModel<KeyType> precomputed_model(existing_node->model_);
    ...
    public:
        ...
        typename self_type::Iterator lower_bound(const KeyType &key) {
            stats_.num_lookups++;
            data_node_type *leaf = get_leaf(key);
        ...
        typename self_type::ConstIterator lower_bound(const KeyType &key) const {
            stats_.num_lookups++;
            data_node_type *leaf = get_leaf(key);
            int idx = leaf->find_lower(key);
        ...
        typename self_type::Iterator upper_bound(const KeyType &key) {
            stats_.num_lookups++;
            data_node_type *leaf = get_leaf(key);
        ...
        typename self_type::ConstIterator upper_bound(const KeyType &key) const {
            stats_.num_lookups++;
            data_node_type *leaf = get_leaf(key);
            int idx = leaf->find_upper(key);
            return ConstIterator(leaf, idx);  // automatically handles the case where
        ...
        std::pair<ConstIterator, ConstIterator> equal_range(const KeyType &key) const {
        ...
        P *get_payload(const KeyType &key) const {
            stats_.num_lookups++;
            data_node_type *leaf = get_leaf(key);
            int idx = leaf->find_key(key);
            if (idx < 0) {
        ...
        typename self_type::Iterator find_last_no_greater_than(const KeyType &key) {
            stats_.num_lookups++;
            data_node_type *leaf = get_leaf(key);
        ...
        // This avoids the overhead of creating an iterator
        P *get_payload_last_no_greater_than(const KeyType &key) {
            stats_.num_lookups++;
            data_node_type *leaf = get_leaf(key);
            const int idx = leaf->upper_bound(key) - 1;
            if (idx >= 0) {
    ...
    public:
        ...
        // found.
        std::pair<Iterator, bool> insert(const KeyType &key, const P &payload) {
...
//                    this->LogHdr3End();
                    ...
                    if (experimental_params_.splitting_policy_method == 0 || fail >= 2) {
                        ...
                        // 2
                        fanout_tree_depth = fanout_tree::find_best_fanout_existing_node<KeyType, P>(
    ...
    private:
        ...
        int best_split_propagation(const std::vector<TraversalNode> &traversal_path,
            ...
            for (const TraversalNode &tn: traversal_path) {
                double stop_cost;
                AlexNode<KeyType, P> *next = tn.node->children_[tn.bucketID];
        ...
        // a new root node.
        void expand_root(KeyType key, bool expand_left) {
            ...
            KeyType domain_size = istats_.key_domain_max_ - istats_.key_domain_min_;
            int expansion_factor;
            KeyType new_domain_min = istats_.key_domain_min_;
            KeyType new_domain_max = istats_.key_domain_max_;
            ...
            if (expand_left) {
                ...
                KeyType half_expandable_domain =
                        istats_.key_domain_max_ / 2 - std::numeric_limits<KeyType>::lowest() / 2;
                KeyType half_expanded_domain_size = expansion_factor / 2 * domain_size;
                if (half_expandable_domain < half_expanded_domain_size) {
                    new_domain_min = std::numeric_limits<KeyType>::lowest();
                } else {
                ...
                // this check, we do comparisons using half of the relevant quantities.
                KeyType half_expandable_domain =
                        std::numeric_limits<KeyType>::max() / 2 - istats_.key_domain_min_ / 2;
                KeyType half_expanded_domain_size = expansion_factor / 2 * domain_size;
                if (half_expandable_domain < half_expanded_domain_size) {
                    new_domain_max = std::numeric_limits<KeyType>::max();
                } else {
            ...
            if (static_cast<size_t>(root->num_children_) * expansion_factor <=
                ...
                auto new_children = new(pointer_allocator().allocate(new_num_children))
                        AlexNode<KeyType, P> *[new_num_children];
                int copy_start;
                if (expand_left) {
                ...
                new_root->children_ = new(pointer_allocator().allocate(expansion_factor))
                        AlexNode<KeyType, P> *[expansion_factor];
                if (expand_left) {
            ...
            if (expand_left) {
                KeyType left_boundary_value = istats_.key_domain_min_;
                ...
                KeyType right_boundary_value = istats_.key_domain_max_;
        ...
        data_node_type *split_upwards(
                KeyType key, int stop_propagation_level,
            ...
            std::vector<AlexNode<KeyType, P> *> to_delete;  // nodes that need to be deleted
            ...
            AlexNode<KeyType, P> *prev_left_split = left_leaf;
            AlexNode<KeyType, P> *prev_right_split = right_leaf;
            ...
            while (traversal_path[path_idx].node->level_ > stop_propagation_level) {
                ...
                AlexNode<KeyType, P> *left_half_first_child = cur_node->children_[0];
                AlexNode<KeyType, P> *right_half_first_child =
                ...
                // Do the split
                AlexNode<KeyType, P> *next_left_split = nullptr;
                AlexNode<KeyType, P> *next_right_split = nullptr;
                if (double_left_half) {
                    // double left half
                    ...
                    if (pull_up_right_child) {
                        ...
                        right_split->children_ =
                                new(pointer_allocator().allocate(right_split->num_children_))
                                        AlexNode<KeyType, P> *[right_split->num_children_];
    ...
    public:
        ...
        int erase_one(const KeyType &key) {
            data_node_type *leaf = get_leaf(key);
    ...
    private:
        ...
        void merge(data_node_type *leaf, KeyType key) {
    ...
    public:
        ...
        long long data_size() const {
            ...
            std::cout<<"Number of leaf nodes!!!!!!!!!!!!!!!!!!!!!!!!!: "<<stats_.num_data_nodes<<std::endl;
            return size;
        }
        ...
        long long model_size() const {
            ...
            std::cout<<"Number of all nodes!!!!!!!!!!!!!!!!!!!!!!!!!: "<<stats_.num_data_nodes + stats_.num_model_nodes<<std::endl;
            return size;
        }
    ...
    public:
        ...
        bool validate_structure(bool validate_data_nodes = false,
            ...
            bool is_valid = true;
            std::stack<AlexNode<KeyType, P> *> node_stack;
            AlexNode<KeyType, P> *cur;
                    ...
                    if (validate_data_nodes) {
                        ...
                        if (node->num_keys_ > 0) {
                            ...
                            if (prev_nonempty_leaf) {
                                KeyType last_in_prev_leaf = prev_nonempty_leaf->last_key();
                                KeyType first_in_cur_leaf = node->first_key();
                            ...
                            if (next_nonempty_leaf) {
                                KeyType first_in_next_leaf = next_nonempty_leaf->first_key();
                                KeyType last_in_cur_leaf = node->last_key();
    ...
    V& operator*() const { return cur_leaf_->data_slots_[cur_idx_]; }
            ...
            const KeyType &key() const { return cur_leaf_->get_key(cur_idx_); }
    ...
    const V& operator*() const { return cur_leaf_->data_slots_[cur_idx_]; }
        ...
        public:
            ...
            AlexNode<KeyType, P> *cur_node_;
            std::stack<AlexNode<KeyType, P> *> node_stack_;  // helps with traversal
            ...
            AlexNode<KeyType, P> *current() const { return cur_node_; }

            AlexNode<KeyType, P> *next() {

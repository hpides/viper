...
#if !defined(xindex_buffer_Hh)
#define xindex_buffer_Hh

namespace xindexh {
const uint8_t alt_buf_fanout = 4;
...
class AltBtreeBuffer {
 ...
 public:
  ...
  inline void replace_pointer(const key_t &key);
  inline bool update(const key_t &key, const val_t &val);
  inline void insert(const key_t &key, const val_t &val);
  inline void insert_ptr(const key_t &key, atomic_val_t *val_ptr);
 ...
 private:
  ...
  void insert_leaf_ptr(const key_t &key, atomic_val_t *val_ptr, leaf_t *target);
  void split_n_insert_leaf(const key_t &key, const val_t &val, int slot,
                           leaf_t *target);
  void split_n_insert_leaf(const key_t &key, atomic_val_t *val_ptr, int slot,
                           leaf_t *target);

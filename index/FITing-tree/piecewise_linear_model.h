...
public:
    ...
    CanonicalSegment get_segment(size_t number) {
        if (points_in_hull == 1)
            return CanonicalSegment(rectangle[0], rectangle[1], first_x);
        return CanonicalSegment(rectangle, first_x, number);
    }
...
class OptimalPiecewiseLinearModel<X, Y>::CanonicalSegment {
    ...
    CanonicalSegment(const Point &p0, const Point &p1, X first) : rectangle{p0, p1, p0, p1}, first(first), number(1) {};

    CanonicalSegment(const Point (&rectangle)[4], X first, size_t number)
            : rectangle{rectangle[0], rectangle[1], rectangle[2], rectangle[3]}, first(first), number(number) {};

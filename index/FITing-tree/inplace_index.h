...
class InplaceIndex : public viper::index::BaseIndex<K>{
protected:

    using NODE = InPlaceNode<K, P>;
...
public:
    ...
    InplaceIndex(K* keys, P* payloads, size_t number)
        :   min_key(*keys),
...
template<typename K, typename P>
struct InplaceIndex<K, P>::Buffer {
...
template<typename K, typename P>
struct InplaceIndex<K, P>::Segment {
    K first_key;
    double slope;
    double intercept;
    size_t number;

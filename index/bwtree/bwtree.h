...
class BwTreeBase {
  ...
  static constexpr size_t CACHE_LINE_MASK = ~(CACHE_LINE_SIZE_ - 1);
  ...
  static_assert(sizeof(GCMetaData) < CACHE_LINE_SIZE_,
  ...
  using PaddedGCMetadata = PaddedData<GCMetaData, CACHE_LINE_SIZE_>;
 ...
 public:
  ...
  void PrepareThreadLocal() {
    ...
    original_p = static_cast<unsigned char *>(
      malloc(CACHE_LINE_SIZE_ * (thread_num + 1)));
    assert(original_p != nullptr);
    
    // Align the address to cache line boundary
    gc_metadata_p = reinterpret_cast<PaddedGCMetadata *>(
      (reinterpret_cast<size_t>(original_p) + CACHE_LINE_SIZE_ - 1) & \
        CACHE_LINE_MASK);
    
    // Make sure it is aligned
    assert(((size_t)gc_metadata_p % CACHE_LINE_SIZE_) == 0);
    
    // Make sure we do not overflow the chunk of memory
    assert(((size_t)gc_metadata_p + thread_num * CACHE_LINE_SIZE_) <= \
             ((size_t)original_p + (thread_num + 1) * CACHE_LINE_SIZE_));

...
namespace viper {
    ...
    static constexpr size_t TWO_MB = 2 * 1024 * 1024;
...
#define STORE_ORDER std::memory_order_release
    ...
    namespace internal {
        ...
        struct VarSizeEntry {
            ...
            VarSizeEntry(const uint64_t keySize, const uint64_t valueSize)
                    : is_set{true}, key_size{static_cast<uint16_t>(keySize)},
        ...
        struct alignas(PAGE_SIZE) ViperPage<std::string, std::string> {
            ...
            static constexpr size_t METADATA_SIZE =
                    sizeof(version_lock) + sizeof(next_insert_offset) + sizeof(modified_percentage);
    ...
    class Viper {
        ...
        using KVOffset = index::KeyValueOffset;
    ...
    public:
        ...
        Viper(ViperBase v_base, std::filesystem::path pool_dir, bool owns_pool, ViperConfig v_config, int index_type);
    ...
    template<typename K, typename V>
    Viper<K, V>::Viper(ViperBase v_base, const std::filesystem::path pool_dir, const bool owns_pool,
                       const ViperConfig v_config, int index_type) :
            v_base_{v_base}, owns_pool_{owns_pool}, v_config_{v_config}, pool_dir_{pool_dir},
...
/*
    ...
    template<typename K, typename V>
    index::KeyValueOffset Viper<K, V>::get_new_block() {
 ...
 */
    ...
    template<typename K, typename V>
    void
    Viper<K, V>::Client::free_occupied_slot(const KVOffset offset_to_delete, const K &key, const bool delete_offset) {

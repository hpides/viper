...
#define IS_BIT_SET(variable, mask) ((variable & mask) != 0)
    ...
    inline bool CAS(KeyType *key, KeyType *expected, KeyType updated) {
        ...
        else if constexpr (sizeof(KeyType) == 2)
            return internal_cas((int16_t *) key, (int16_t *) expected, (int16_t) updated);
        else if constexpr (sizeof(KeyType) == 4)
            return internal_cas((int32_t *) key, (int32_t *) expected, (int32_t) updated);
        else if constexpr (sizeof(KeyType) == 8)
            return internal_cas((int64_t *) key, (int64_t *) expected, (int64_t) updated);
        else if constexpr (sizeof(KeyType) == 16)
            return internal_cas((__int128 *) key, (__int128 *) expected, (__int128) updated);
        else throw std::runtime_error("CAS not supported for > 16 bytes!");
    }


    using IndexK = size_t;
    using IndexV = index::KeyValueOffset;
...
#define CACHE_LINE_SIZE 64
        ...
        template<typename KeyType>
        class CCEH : public index::BaseIndex<KeyType> {
        public:
        ...
        void CCEH<KeyType>::Remove(IndexV *offset) {
            index::offset_size_t expected_value = offset->offset;
